	...
	$allModules
		.each(function(){
			var
				...
				lowerBound   = 0,
				upperBound   = parseInt($module.css('width'), 10),
				...
				bind: {
					...
					input: function(bindingSelector) {
						...
						if ($dropdown !== undefined) {
							...
							/* In addition, after slider is destroyed, onChange is restored to default
				...
				event: {
					thumb: {
						...
						moved: function(event) {
							if (dragging) {
								var position = parseInt($thumb.css("left"), 10);
...
$.fn.slider.settings = {
	...
	// multiple sliders binded to a input are also binded together if this value is set to true
